...
* **序列化ID**
     - 作用
          * Java 序列化機制是通過在運行時判斷類的serialVersionUID 驗證版本是否一致；
          * 反序列化時，JVM 會把傳來的字節流的serialVersionUID 與本地實體類中的serialVersionUID 進行比較。相同則進行反序列化，反之則拋出異常。


* **序列化 & 反序列化比較**

     - 序列化是把Java 對象轉換為字節序列的過程；
     - 反序列化是指把字節序列恢復為Java 對象的過程；
     - 序列化的作用是：傳遞和保存對象的時候，保證對象的完整性和可傳遞性。 把對象轉換成有序字節流，以便在網絡上傳輸或者保存在本地文件中。序列化的核心作用是：對象狀態的保存。
     - 反序列化：根據字節流中所保存的對象狀態及描述信息，通過反序列化重建對象， 恢復對象的狀態。
     - 序列化和反序列化的用途：
          * 數據持久化：將數據永久地保存在磁盤上；
          * 實現遠程通信：以字節流的形式使對象在網絡上進行傳遞和接收；
   
* **注意事項**  
     - 序列化時， 只對對象的狀態進行保存， 不管對象的方法；
     - 父類實現序列化，子類自動實現序列化， 不需要顯式地實現Serialzable 接口；
     - 當一個對象的實例變量引用其他對象，序列該對象時也把引用對象進行序列化；
     - 並非所有的對象都能被序列化；
     - 聲明為static ，transient 類型的成員數據不能被序列化。static: 代表類的狀態； transient: 對象的臨時數據；
